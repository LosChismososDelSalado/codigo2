...
function desactivarModoNina() {
    ...
    // Restaurar zonas explícitamente con 'auto' — no depender del CSS
    document.querySelectorAll('.zona').forEach(z => {
        z.style.pointerEvents = 'auto';
    });
    // Un frame después limpiar el inline para dejar el CSS en control
...
const GalaxiaCorazones = (() => {
    ...
    function activarGalaxia() {
        ...
        // ── CAPAS DEL MURAL: fondo negro puro — todas las capas ocultas ──
        document.querySelectorAll('.layer').forEach(l => {
            l.dataset.filtroAnterior   = l.style.filter   || '';
            l.dataset.opacidadAnterior = l.style.opacity  || '';
            l.style.transition         = 'filter 0.4s ease, opacity 0.4s ease';
            l.style.filter             = 'brightness(0)';
            l.style.opacity            = '0';
        });
    ...
    function cerrarGalaxia() {
        ...
        // Restaurar zonas: primero 'auto' explícito, luego limpiar inline
        document.querySelectorAll('.zona').forEach(z => {
            z.style.pointerEvents = 'auto';
        });
        requestAnimationFrame(() => {
